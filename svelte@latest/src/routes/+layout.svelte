<script lang="ts">
  ...
  function handleRouteProtection() {
    ...
    console.log('Root layout handling route protection for path:', currentPath);

    // 如果用户已登录但访问的是登录或注册页面，重定向到 doing 页面
    if ($isAuthenticated && (currentPath === '/login' || currentPath === '/register')) {
      console.log('User is authenticated and trying to access login/register, redirecting to /doing');
      goto('/doing');
      return;
    }
    ...
    if (!currentPath || currentPath === '/') {
      if ($isAuthenticated) {
        console.log('Empty path with authenticated user, redirecting to /doing');
        goto('/doing');
      } else {
        console.log('Empty path with unauthenticated user, redirecting to /login');
        goto('/login');
      }
      return;
    }
