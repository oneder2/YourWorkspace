 ...
 * API Cache Service
 * 
 * This module provides caching functionality for API requests to improve performance
...
export async function getWithCache<T>(
  ...
  if (cachedData) {
    ...
    if (mergedOptions.backgroundRefresh && browser && navigator.onLine) {
      setTimeout(async () => {
        try {
          const freshData = await api.get<T>(endpoint, requiresAuth);
  ...
  try {
    const data = await api.get<T>(endpoint, requiresAuth);
...
/**
 * Prefetches data for common routes
 * This can be called during app initialization or when navigating to a new route
 * 
 * @param route - The route to prefetch data for
 */
export function prefetchRouteData(route: string): void {
  if (!browser) return;
  
  // Map routes to endpoints that should be prefetched
  const routeEndpointMap: Record<string, string[]> = {
    '/doing': ['/todos'],
    '/done': ['/achievements'],
    '/plan': ['/plans'],
    '/anchor': ['/profile']
  ...
  // Get endpoints to prefetch for this route
  const endpoints = routeEndpointMap[route] || [];
  
  // Prefetch each endpoint
  endpoints.forEach(endpoint => {
    prefetchData(endpoint, true, {
