<script lang="ts">
  ...
  <div
    role="button"
    tabindex="0"
    class="bg-white dark:bg-gray-800 p-3 mb-2 transition-all hover:bg-green-50 dark:hover:bg-green-900/20 border-l-4 border-green-400 dark:border-green-600 cursor-pointer"
    onclick={() => onClick(futurePlan)}
    onkeydown={(e) => e.key === 'Enter' && onClick(futurePlan)}
  >
    <div class="flex justify-between items-start">
      <h4 class="text-sm font-semibold text-green-600 dark:text-green-400 flex-grow pr-2 truncate">
        {futurePlan.description}
      </h4>
      <div class="flex space-x-1 rtl:space-x-reverse flex-shrink-0">
        <button
          onclick={(e) => { e.stopPropagation(); onEdit(futurePlan); }}
          aria-label="Edit plan: {futurePlan.description}"
          class="p-1 text-xs font-medium text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300 focus:outline-none focus:ring-1 focus:ring-green-500 rounded-md mr-1"
        >
        ...
        <button
          ...
          class="p-1 text-xs font-medium text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 focus:outline-none focus:ring-1 focus:ring-red-500 rounded-md disabled:opacity-50"
        >
          {#if isDeleting}
    ...
    <div class="flex flex-wrap gap-2 mt-1 text-xs text-gray-600 dark:text-gray-400">
      ...
      {#if futurePlan.target_date}
        <span class="px-2 py-0.5 bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300 rounded-full">
          {formatDate(futurePlan.target_date)}
        </span>
      {/if}

      {#if futurePlan.goal_type}
        <span class="px-2 py-0.5 bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-300 rounded-full">
