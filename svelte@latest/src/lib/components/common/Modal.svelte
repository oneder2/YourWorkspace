<script lang="ts">
    ...
    $: if (isOpen && typeof document !== 'undefined') {
      ...
      // Wait for modal to be rendered then focus it or its first focusable element
      tick().then(() => {
        if (modalContentElement) {
          ...
          if (firstFocusable) {
            ...
            // Fallback to focusing the modal content wrapper itself if no focusable elements inside
            // Ensure modalContentElement has tabindex="-1" if this is desired.
            // For now, the backdrop has tabindex="-1".
          }
        }
      });
      ...
      if (previousActiveElement) {
          // Delay focus restoration slightly to avoid race conditions with transitions
  ...
  {#if isOpen}
    <div
      ...
      <div
        ...
        role="document" transition:fly={{ y: -30, duration: 300 }}
        aria-labelledby={title ? 'modal-title' : undefined} >
  ...
  <style>
    /* Ensure body.overflow-hidden-modal is in global.css */
    /*
